...
class InclusionGate(PropertyGatingType):
    def __init__(
        ...
        super().__init__(
            property_type=None, entity_property=entity_property, allow=allow
        )
    ...
    def _parse_json_params(cls, gate_json: Dict[str, Any]) -> Dict:
        ...
        if "valid_values" not in gate_json or not isinstance(
            gate_json["valid_values"], list
        ):
            raise ValueError("'valid_values' field must be provided and be a list")
